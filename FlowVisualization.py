...
for t in range(len(attention_maps) - 1):
    ...
    frame_t1 = np.where(frame_t1 >= 0.5 * np.max(frame_t1), frame_t1, 0)
    frame_t = np.where(frame_t >= 0.5 * np.max(frame_t), frame_t, 0)
    H, W = frame_t.shape
    max_loc_t1 = np.unravel_index(np.argmax(frame_t1, axis=None), frame_t1.shape)
    max_y, max_x = max_loc_t1
    y_coords, x_coords = np.meshgrid(np.arange(H), np.arange(W), indexing="ij")
    # print(H, W, max_y, max_x)
    delta_x = max_x - x_coords
    delta_y = max_y - y_coords
    flow_tensor = np.stack((delta_x, delta_y), axis=-1)
    flow_tensors.append(flow_tensor)
    plt.imshow(frame_t1)
    plt.scatter(max_x, max_y, c='r')
    plt.show()
    # plt.imsave(os.path.join("Flows_dog", f"{t}{t + 1}-H.png"), flow_tensor[:, :, 0], cmap='viridis')
    # plt.imsave(os.path.join("Flows_dog", f"{t}{t + 1}-V.png"), flow_tensor[:, :, 1], cmap='viridis')
